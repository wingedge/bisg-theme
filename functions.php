...
function bi_display_popular_videos($args=array()){
    $default = array(
        ...
        'order'             => 'DESC',
        'orderby'           => 'post_date',
        'category_name'     => 'featured-video',//'most-popular-video',    
...
function bi_new_user_email($wp_new_user_notification_email, $user){
    ...
    */
    // new changes to email
    ob_start();
    include 'section/newsletter-register.php';
...
function bi_new_user_email_reset_password_message( $message, $key ) {
    ...
    $msg = __('<center><a href="' . network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . '"><img src="http://beautyinsider.sg/wp-content/uploads/2018/02/PW-reset-request.png" style="width: 70%"></a></center>');
...
function wpse_lost_password_redirect() {
    ...
    if( $confirm ) {
        wp_redirect( home_url('/password-has-been-reset/') ); 
        exit;
    }
}
...
function wplogin_login_message( $message ) {
    if ( empty($message) ){
        return "<h1 class='wplogin_lm_title'>Welcome back, gorgeous!</h1><span class='wplogin_lm_sub-title'>Login and get a chance to enjoy freebies and rewards!</span>";
    } else {
        return $message;
    }
}
add_filter( 'login_message', 'wplogin_login_message' );


add_action('wp_ajax_share_add_points', 'share_add_points');
add_action('wp_ajax_nopriv_share_add_points', 'share_add_points');
function share_add_points(){
    global $BIReview;

    $url = $_POST['shareUrl'];
    $service = $_POST['shareService'];

    $remark = 'shared via '.$service.' '.$url;

    $BIReview->check_and_add_points($remark,20);
...
// slug used for forms/shortcode
function bi_get_current_user($atts){
...
add_shortcode('bi_get_current_user', 'bi_get_current_user');

// trial team form, force one time submission only

add_action( 'wpcf7_before_send_mail', 'bi_trial_team_form_check' );

function bi_trial_team_form_check($contact_form){ 
 $uid = $_POST['tt_uid'];
 $form_id = $_POST['_wpcf7'];
    if ($form_id == 4787){ // 123 => Your Form ID.   
        update_user_meta($uid, 'joined_trial_team', 'joined trial team');
    }     
}
...
add_action( 'wp_ajax_auth_vote_user', 'auth_vote_user' );
add_action( 'wp_ajax_nopriv_auth_vote_user', 'auth_vote_user' );


function auth_vote_user() {
    ...
    if ( !is_wp_error( $user ) ){     
        $json = array('success' => true, 'result' => $user->data);        
        $output = json_encode($json,JSON_HEX_QUOT);
        echo $output;        
    }
    else {
